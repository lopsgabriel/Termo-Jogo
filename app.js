...
function adicionar_letra() {
    ...
    if (caixaAtiva.value && letter) {
        caixaAtiva.value.textContent = letter; // Coloca a letra na caixinha ativa
        ...
        if (next_box && next_box.classList.contains('letter-box')) {
            ...
            caixaAtiva.value.classList.remove('letter-edit'); // Se não houver próxima, remove o destaque
            caixaAtiva.value = null; // Desativa a seleção
        }
    } else {
